...
## 3. New rules
The introduction of new rules (introduced by an existing analyzer or by a newly added analyzer) will potentially report a lot of issues. They might be valid points, but you don't necessarily want to halt the release of all features until all issues are solved. By (temporarily) disabling such rules, newly added code will not yet benefit from this rule.
...
Some extensions in Visual Studio (like VS Spell Checker or SonarQube IDE) add warnings to the build output as they are de facto analyzers. As they are not configured by the solution but by the environment of the developer, those developers can not build the solution.

## 6. False positives
Although analyzer rule builders - like us - try their best to come up with robust and correct analyzers, it turns out that now and then a rule reports a false positive. In such cases it is better to not suppress the issue, or adjust the code just to satisfy the rule, but to file a bug report and wait for an update that fixes the FP.
